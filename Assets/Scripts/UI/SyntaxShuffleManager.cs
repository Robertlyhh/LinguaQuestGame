...
public class SyntaxShuffleManager : MonoBehaviour
{
    ...
    public TextMeshProUGUI questionText; // UI element to display the question
    public TextMeshProUGUI feedbackText; // UI element to display feedback
    public List<WordOrderQuestion> wordOrderQuestions; // List of predefined word order questions
    ...
    void Start()
    {
        LoadSentence(wordOrderQuestions[currentQuestionIndex]);
    }
    ...
    void Update()
    {
        if (timerRunning && !answered)
        {
            ...
            if (timeRemaining <= 0)
            {
                ...
                battleManager.OnPlayerSubmitted(false, 0f, wordOrderQuestions[currentQuestionIndex].explanation);
            }
        }
    }
    ...
    public void LoadSentence(WordOrderQuestion wordOrderQuestion)
    {
        ...
        correctSentence = wordOrderQuestion.wordParts != null ? new List<string>(wordOrderQuestion.wordParts) : new List<string>();
        ...
        questionText.text = wordOrderQuestion.questionText; // Set the question text
    }
    ...
    public void SubmitAnswer()
    {
        ...
        StartCoroutine(battleManager.OnPlayerSubmitted(correct, timeRemaining, wordOrderQuestions[currentQuestionIndex].explanation));
    ...
    public void StartNewChallenge(BattleManager bm)
    {
        battleManager = bm;
        answered = false; // Reset answered state for new challenge
        currentQuestionIndex = (currentQuestionIndex + 1) % wordOrderQuestions.Count;
        LoadSentence(wordOrderQuestions[currentQuestionIndex]);
